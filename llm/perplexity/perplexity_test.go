...
// Perplexity is a search-augmented provider: responses carry a top-level
// "citations" list of source URLs, which the compat layer surfaces as
// langrails.Citation entries.
func TestProvider_Complete_Citations(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		resp := compat.TestResponse{
			Model: "sonar-pro",
			Choices: []compat.TestChoice{{
				Message:      compat.TestMessage{Role: "assistant", Content: "The answer is 42."},
				FinishReason: "stop",
			}},
	...
	if resp.Citations[0].URL != "https://a.com" || resp.Citations[1].URL != "https://b.com" {
